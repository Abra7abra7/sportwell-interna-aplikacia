...
import { updateActiveEmployeeAction } from "../actions";
import { ROLE_OPTIONS } from "./InviteEmployeeModal";

interface EditActiveEmployeeModalProps {
  employee: any;
  onClose: () => void;
}

export default function EditActiveEmployeeModal({ employee, onClose }: EditActiveEmployeeModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateActiveEmployeeAction(employee.id, employee.metadata, {
        fullName,
        phone,
        roleTitle
      });
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <input disabled type="email" value={employee.email} 
              className="w-full px-4 py-2 border border-gray-100 rounded-xl bg-gray-50 text-gray-400 text-sm cursor-not-allowed" title="Email nie je možné zmeniť" />
          </div>
          ...
          <div className="pt-6 mt-6 border-t border-gray-100 flex justify-end space-x-3 shrink-0">
            ...
            <button type="submit" disabled={loading} className="px-5 py-2.5 bg-brand-cyan hover:shadow-md text-brand-dark-navy rounded-xl font-bold transition-all disabled:opacity-50 text-sm min-h-[44px]">Uložiť zmeny</button>
          </div>
        </form>
      </div>
    </div>
