...
export default function InviteEmployeeModal({ onClose }: InviteEmployeeModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.message || "Chyba pri odosielaní pozvánky.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="pt-6 mt-6 border-t border-gray-100 flex justify-end space-x-3 shrink-0">
            <button type="button" onClick={onClose} className="px-5 py-2.5 text-gray-600 hover:bg-gray-100 rounded-xl font-medium transition-colors text-sm min-h-[44px]">
              Zrušiť
            </button>
            <button type="submit" disabled={loading} className="px-5 py-2.5 bg-brand-cyan hover:shadow-md text-brand-dark-navy rounded-xl font-bold transition-all disabled:opacity-50 text-sm min-h-[44px]">
