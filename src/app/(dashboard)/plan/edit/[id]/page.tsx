...
export default function EditPlanPage() {
      ...
      if (planId) {
        const { data: planData, error: planError } = await supabase
          ...
          .select(`
            *,
            plan_exercises (
              id, order_index, target_sets, target_reps, target_duration, target_rest_seconds, notes,
        ...
        if (planError) {
          alert("Chyba pri načítaní plánu.");
          router.push("/plan");
          return;
        }
        ...
        if (planData) {
          ...
          setWarmupNotes(planData.warmup_notes || "");
          setSelectedClientId(planData.client_id);
          
          if (planData.plan_exercises) {
            // Zoradiť podľa order_index
            const sorted = [...planData.plan_exercises].sort((a, b) => a.order_index - b.order_index);
            
            const mappedExercises = sorted.map((pe: any) => ({
              tempId: pe.id || Math.random().toString(36).substr(2, 9),
              ...
              rpe: pe.rpe || "80%",
              rest_between_exercises: pe.rest_between_exercises || 0,
              notes: pe.notes || ""
            }));
            setPlanExercises(mappedExercises);
          }
        }
      }
  ...
  if (loadingData) {
    return <div className="flex justify-center p-20"><div className="animate-spin h-12 w-12 border-t-2 border-brand-cyan rounded-full"></div></div>;
  }
