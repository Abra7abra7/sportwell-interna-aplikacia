...
export default function SeedPage() {
  ...
  const handleSeed = async () => {
    setStatus("Aktualizujem šablónu 'Základná diagnostika'...");
    ...
    if (error) {
      ...
      setStatus("Hotovo! Schéma bola úspešne nahratá. Môžeš sa vrátiť do Diagnostiky.");
    }
  };

  return (
    <div className="p-8 max-w-lg mx-auto bg-white rounded-lg shadow mt-10">
      <h1 className="text-2xl font-bold mb-4">Aktualizácia šablón</h1>
      <p className="mb-6 text-gray-600">
        Kliknutím na tlačidlo nižšie sa zapíše správny dlhý JSON formát pre Základnú diagnostiku do databázy (vyžaduje prihlásenie).
      </p>
      <button 
        onClick={handleSeed}
        className="w-full bg-brand-navy text-white font-bold py-3 px-4 rounded"
      >
        Spustiť aktualizáciu
      </button>
      {status && <p className="mt-4 p-3 bg-gray-100 rounded text-brand-navy font-bold">{status}</p>}
    </div>
  );
}
